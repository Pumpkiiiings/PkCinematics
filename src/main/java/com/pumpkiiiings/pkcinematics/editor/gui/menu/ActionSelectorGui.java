...
public class ActionSelectorGui {
                    ...
                    switch(text.toLowerCase()) {
                        ...
                        default:
                            try {
                                ...
                                player.sendMessage("§cInvalid value, using 0 (dawn).");
                                t = 0;
                            }
                            break;
                    }
    ...
    private static void askTickAndExecute(Player player, EditorSession session, Consumer<Integer> onSuccess) {
        PkCinematics.getApi().getChatInputManager().requestInput(player, "§aIn which TICK do you want to execute this action?", input -> {
            try {
